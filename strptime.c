...
start:
        switch (*p++) {
          ...
          case 'F':
            ...
            break;
          case 'g':
            if (!parse_digits(&s, 2, &tm->tm_isoyy)) return NULL;
            break;
          case 'G':
            if (!parse_digitsv(&s, width ? width : 4, &tm->tm_isoyear)) return NULL;
            break;
          case 'H':
            if (!parse_digits(&s, 2, &tm->tm_hour)) return NULL;
            break;
          ...
          case 'u':
            ...
            break;
          case 'U':
            if (!parse_digits(&s, 2, &tm->tm_weekU)) return NULL;
            break;
          case 'w':
            if (!parse_digits(&s, 1, &tm->tm_wday)) return NULL;
            break;
          case 'W':
            if (!parse_digits(&s, 2, &tm->tm_weekW)) return NULL;
            break;
          case 'y':
            if (!parse_digits(&s, 2, &tm->tm_yy)) return NULL;
            break;
